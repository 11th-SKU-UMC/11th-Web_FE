...
function getMemberInfo(id: number): string {
  ...
  if (!member) return "존재하지 않는 회원입니다.";
  const githubId = member.githubId ?? "등록되지 않음";
  return `이름: ${member.name}, 역할: ${member.role}, GitHub ID: ${githubId}`;
}
...
function formatMemberId(input: unknown): string {
  if (typeof input === "number") return `숫자 ID: ${input}`;
  if (typeof input === "string") return `문자열 ID: ${input}`;
